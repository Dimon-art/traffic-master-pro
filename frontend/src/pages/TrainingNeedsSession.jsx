/** Диалог сценария «Выявление потребностей». */
...
function TrainingNeedsSession() {
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      try {
        const data = await startTraining('needs', topic)
        if (cancelled) {
          return
        }
        setTraining(data)
  ...
  const roundNumber = Math.min((training?.current_question_index || 0) + 1, 7)
  const lastUserMessage = [...messages].reverse().find((message) => message.role === 'user')
  const clientReady = (training?.score ?? 0) >= 60
    ...
    <>
      ...
      <main style={{ paddingTop: '72px', minHeight: '100vh' }}>
        <Section>
          ...
          >
            <div>
              <h1 style={{ fontSize: theme.sizes.h2 }}>Выявление потребностей</h1>
              {!completed && !loading ? (
                <p style={{ color: theme.colors.textMuted, margin: `${theme.sizes.xs} 0 0` }}>
                  Раунд {roundNumber} из 7
                </p>
              ) : null}
            </div>
            {!completed ? (
          ...
          {completed ? (
            <div
              style={{
                ...
                background: clientReady ? theme.colors.successSoft : theme.colors.errorSoft,
                border: `1px solid ${clientReady ? theme.colors.success : theme.colors.error}`,
                borderRadius: theme.radii.lg,
                textAlign: 'center',
                color: clientReady ? theme.colors.success : theme.colors.error,
              }}
            >
              <h2 style={{ fontSize: theme.sizes.h3 }}>
                {clientReady ? 'Клиент готов к предложению' : 'Клиент ушёл'}
              </h2>
              <p style={{ margin: `${theme.sizes.sm} 0 ${theme.sizes.md}` }}>
              ...
              <div style={{ display: 'flex', gap: theme.sizes.sm, justifyContent: 'center', flexWrap: 'wrap' }}>
                ...
                <Button variant="secondary" onClick={() => navigate('/training/needs')}>
                  Другой сценарий
                </Button>
              </div>
            </div>
            ...
            training && (
              <form onSubmit={handleSubmit} style={{ marginTop: theme.sizes.md }}>
                <textarea
                  ...
                  rows={4}
                  required
                  placeholder="Задайте открытый вопрос клиенту"
                  style={fieldStyle}
                />
                <div style={{ marginTop: theme.sizes.sm }}>
                  <Button type="submit" variant="primary" disabled={submitting}>
                    {submitting ? 'Проверяю...' : 'Спросить'}
                  </Button>
                </div>
              </form>
            )
...
export default TrainingNeedsSession
