...
## Использование ИИ (DeepSeek)

Проект может работать в двух режимах.

ИИ используется во всех трёх доступных режимах:
- «Знание продукта»: оценка ответов через DeepSeek
- «Работа с возражениями»: клиент + оценка через DeepSeek
- «Выявление потребностей»: клиент + оценка через DeepSeek

При USE_AI=false или ошибке — fallback на мок-логику по keywords.

### Мок-режим (по умолчанию)
- USE_AI=false в .env
- Оценка ответов по ключевым словам
- Клиенты в диалогах отвечают по сценарию из JSON
- Бесплатно, быстро, стабильно
- Используется в автотестах

### Режим ИИ
- USE_AI=true в .env
- Нужен DEEPSEEK_API_KEY (platform.deepseek.com)
- Оценка ответов через GPT (умнее, учитывает смысл)
- Клиенты ведут живой диалог
- Стоимость: ~$0.002 за тренировку (~$2 на 1000 тренировок)
...
### Fallback
Если ИИ недоступен (нет ключа, кончился баланс, ошибка сети) —
система автоматически возвращается к мок-логике. Тренировка не
падает.
